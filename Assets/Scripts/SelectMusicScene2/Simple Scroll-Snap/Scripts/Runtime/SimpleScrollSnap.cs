...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        public UnityEvent onPanelChanged, onPanelSelecting, onPanelSelected, onPanelChanging,onBeginSelecting;
        ...
        public MusicItem Panel => Panels[CurrentPanel];

        private RectTransform[] PanelsRT
        { get; set; }
        public MusicItem[] Panels { get; set; }
        ...
        public void PublicInitialize()
        {
            if (Validate())
            {
                ...
                {
                    if(Panels[i].name == Setting.I.LastPlayedMusic.MusicId.ToString())
                    {
                        n = i;
                    }
        ...
        private void Initialize()
        {
            ...
            //スペーシングはendSpasingも揃える
            infiniteScrollingEndSpacing = automaticLayoutSpacing;
            if (canvas != null)
            {
        ...
        private void Setup()
        {
            ...
            Panels = new MusicItem[NumberOfPanels];
            PanelsRT = new RectTransform[NumberOfPanels];
            for (int i = 0; i < NumberOfPanels; i++)
            {
                Panels[i] = Content.GetChild(i).GetComponent<MusicItem>();
        ...
        private void OnOcclusionCulling(bool forceUpdate = false)
        {
            if (useOcclusionCulling && (velocity.magnitude > 0f || forceUpdate))
            {
                for (int i = 0; i < NumberOfPanels; i++)
                {
                    if (movementAxis == MovementAxis.Horizontal)
                    {
                        Panels[i].gameObject.SetActive(Mathf.Abs(DisplacementFromCenter(i).x) <= Viewport.rect.width / 2f + size.x);
                    }
                    else if (movementAxis == MovementAxis.Vertical)
                    {
                        Panels[i].gameObject.SetActive(Mathf.Abs(DisplacementFromCenter(i).y) <= Viewport.rect.height / 2f + size.y);
                    }
                }
            }
        }
        ...
        public void Add(GameObject panel, int index)
        {
            ...
            //Instantiateするのではなく、直接このオブジェクトの子オブジェクトとする
